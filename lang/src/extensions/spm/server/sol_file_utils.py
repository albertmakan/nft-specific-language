...
def extract_sol_data(input, input_lines):
    ...
    sourceUnit = parser.parse(input.replace(
        '{{', '"{').replace('}}', '}"'), loc=True)

    global_code = input
    for contract_code in contracts.values():
        global_code = global_code.replace(contract_code, "")
...
def extract_sol_contracts(code):
    contract_start_positions = [match.start(
        0) for match in re.finditer(r"contract .*(is|\{)", code)]
...
def extract_sol_functions(code):
    function_start_positions = [match.start(
        0) for match in re.finditer(r"function .*\(", code)]
...
def extract_sol_modifiers(code):
    modifiers_start_positions = [match.start(
        0) for match in re.finditer(r"modifier .*", code)]
...
def extract_sol_structs(code):
    struct_start_positions = [match.start(
        0) for match in re.finditer(r"struct .*\{", code)]
...
def extract_sol_events(code):
    event_start_positions = [match.start(
        0) for match in re.finditer(r"event .*\(", code)]
...
def extract_state_variables(contractAST, code_lines):
    ...
    for subNode in contractAST["subNodes"]:
        ...
        state_variable_code = extract_text_using_pos(
            code_lines, subNode["loc"]["start"], subNode["loc"]["end"])
