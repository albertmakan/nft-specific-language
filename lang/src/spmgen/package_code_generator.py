...
def compute_initial_queue_item(export):
  export_tokens = export.split('.')

  if export_tokens[-1] == 'contracts':
    return [[export_tokens[0]], export_tokens[0], export_tokens[1]]

  return [[export_tokens[0], '@global'], export_tokens[1], export_tokens[2]]

def find_all_exported_items(sol_data, exports):
  queue = [compute_initial_queue_item(export) for export in exports]
  exported_items = []
